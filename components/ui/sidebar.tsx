...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
  ...
  if (collapsible === "none") {
    return (
      <div
        className={cn(
          "flex h-full w-[--sidebar-width] flex-col bg-slate-50 text-slate-900 border-r border-slate-200",
          className,
        )}
        ref={ref}
        {...props}
  ...
  if (isMobile) {
    return (
      <div className={cn("fixed inset-0 z-50", openMobile ? "block" : "hidden")}>
        ...
        <div
          className={cn(
            "fixed inset-y-0 left-0 z-50 w-[--sidebar-width] bg-slate-50 text-slate-900 shadow-lg border-r border-slate-200",
            className,
          )}
          style={
            {
    ...
    >
      ...
      >
        <div
          ...
          className="flex h-full w-full flex-col bg-slate-50 group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-slate-200 group-data-[variant=floating]:shadow"
        >
          {children}
        </div>
      </div>
...
const SidebarInset = React.forwardRef<HTMLDivElement, React.ComponentProps<"main">>(({ className, ...props }, ref) => {
  return (
    <main
      ref={ref}
      className={cn(
        "relative flex min-h-screen flex-1 flex-col bg-white",
  ...
  ({ className, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        ...
        className={cn(
          "duration-200 flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-slate-500 outline-none ring-slate-950 transition-[margin,opa] ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
  ...
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-slate-950 transition-[width,height,padding] hover:bg-slate-100 hover:text-slate-900 focus-visible:ring-2 active:bg-slate-100 active:text-slate-900 disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-slate-100 data-[active=true]:font-medium data-[active=true]:text-slate-900 data-[state=open]:hover:bg-slate-100 data-[state=open]:hover:text-slate-900 group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        outline:
          "bg-white shadow-[0_0_0_1px_hsl(220_13%_91%)] hover:bg-slate-100 hover:text-slate-900 hover:shadow-[0_0_0_1px_hsl(220_13%_91%)]",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs",
...
const SidebarSeparator = React.forwardRef<React.ElementRef<typeof Separator>, React.ComponentProps<typeof Separator>>(
  ({ className, ...props }, ref) => {
    return (
      <Separator ref={ref} data-sidebar="separator" className={cn("mx-2 w-auto bg-slate-200", className)} {...props} />
    )
  },
)
...
export {
  ...
  SidebarProvider,
  SidebarTrigger,
  SidebarSeparator, // Export SidebarSeparator
  useSidebar,
}
