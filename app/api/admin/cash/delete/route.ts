...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("🗑️ Delete cash voucher request:", { voucher_id, otp: otp ? "***" : "missing" })
    ...
    // Verify OTP first
    const storageKey = `cash_${voucher_id}`
    ...
    const deleteResponse = await fetch(`${apiUrl}/cash-vouchers/${voucher_id}`, {
      method: "DELETE",
      headers: {
    ...
    if (!deleteResponse.ok) {
      ...
      return NextResponse.json(
        {
          message: responseData.message || "Failed to delete cash voucher",
    ...
    console.log("✅ Cash voucher deleted successfully")
